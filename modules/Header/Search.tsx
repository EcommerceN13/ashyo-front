    ...
    <form className="w-[518px] relative">
      ...
      <ul
        className={`${
          ...
          isLoading && "px-5"
        } absolute w-full bg-white top-[67px] sm:top-[39px] pb-[10px] shadow-md`}
      >
        {isLoading ? (
          ...
          categories.map((item: CategoryType) => (
            <li
              key={item.id}
              className="py-[17px] cursor-pointer hover:bg-[#EBFFB3] duration-300 pl-[40px] border-b-[1.5px] border-slate-300"
            >
              {item.name}
            </li>
          ))
